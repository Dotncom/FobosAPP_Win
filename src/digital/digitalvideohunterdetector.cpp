...
constexpr int DIGITAL_VIDEO_HUNTER_MIN_BINS = 4;
constexpr int DIGITAL_VIDEO_HUNTER_MAX_REPORTED_CANDIDATES = 12;

float percentile(std::vector<float> values, double ratio) {
    if (values.empty()) {
        return -160.0f;
    }
    std::sort(values.begin(), values.end());
    const int index = (std::clamp)(static_cast<int>(std::lround((values.size() - 1) * ratio)),
                                   0,
                                   static_cast<int>(values.size()) - 1);
    return values[static_cast<std::size_t>(index)];
}

QString classifyDigitalVideo(double widthMhz, float flatnessDb, float occupancy) {
    if (widthMhz >= 4.0 && flatnessDb <= 8.0f && occupancy >= 0.65f) {
        return QStringLiteral("OFDM-like digital video");
    }
    if (widthMhz >= 2.0 && flatnessDb <= 14.0f && occupancy >= 0.45f) {
        return QStringLiteral("wide digital burst");
    }
    return QStringLiteral("wide digital/unknown");
}

} // namespace

DigitalVideoHunterSettings DigitalVideoHunterDetector::normalizedSettings(DigitalVideoHunterSettings settings) {
...
DigitalVideoHunterResult DigitalVideoHunterDetector::analyze(const std::vector<float> &frequencies,
                                                             const std::vector<float> &magnitudes,
                                                             DigitalVideoHunterSettings settings) {
    settings = normalizedSettings(settings);

    DigitalVideoHunterResult result;
    result.enabled = settings.enabled;

    if (!settings.enabled) {
        result.statusText = QStringLiteral("Digital Video Hunter: off");
        return result;
    }
    ...
    if (dataCount < 32) {
        result.statusText = QStringLiteral("Digital Video Hunter: waiting for spectrum");
        return result;
    }
    ...
    if (frequencyDeltas.empty()) {
        result.statusText = QStringLiteral("Digital Video Hunter: invalid spectrum range");
        return result;
    }
    ...
    if (finiteLevels.empty()) {
        result.statusText = QStringLiteral("Digital Video Hunter: no finite spectrum levels");
        return result;
    }
    ...
    const double gapBreakHz = (std::max)(binHz * 8.0, (std::max)(500000.0, minWidthHz * 0.20));
    ...
    const int smoothRadius = (std::clamp)(static_cast<int>(std::lround(500000.0 / binHz)),
                                          1,
                                          16);
    ...
    auto finishRun = [&](int runEnd) {
        ...
        if (bins < DIGITAL_VIDEO_HUNTER_MIN_BINS) {
            return;
        }
        ...
        const float occupancyThreshold = noiseFloorDb + (thresholdDb - noiseFloorDb) * 0.55f;
        std::vector<float> runLevels;
        runLevels.reserve(static_cast<std::size_t>(bins));
        ...
        const float flatnessDb = percentile(runLevels, 0.90) - percentile(runLevels, 0.10);
        const float occupancy = static_cast<float>(occupiedBins) / static_cast<float>((std::max)(1, bins));
        const double widthMhz = widthHz / 1000000.0;
        const QString type = classifyDigitalVideo(widthMhz, flatnessDb, occupancy);
        const float typeBonus = type.startsWith(QStringLiteral("OFDM")) ? 6.0f
                              : type.startsWith(QStringLiteral("wide digital burst")) ? 3.0f
                              : 0.0f;
        const float score = excessDb * 1.2f +
                            occupancy * 10.0f +
                            static_cast<float>((std::min)(widthMhz, 40.0) * 0.25) -
                            flatnessDb * 0.5f +
                            typeBonus;

        DigitalVideoHunterCandidate candidate;
        ...
        candidate.flatnessDb = flatnessDb;
    ...
    std::sort(result.candidateList.begin(),
              ...
              [](const DigitalVideoHunterCandidate &a, const DigitalVideoHunterCandidate &b) {
                  return a.score > b.score;
              });
    if (static_cast<int>(result.candidateList.size()) > DIGITAL_VIDEO_HUNTER_MAX_REPORTED_CANDIDATES) {
        result.candidateList.resize(DIGITAL_VIDEO_HUNTER_MAX_REPORTED_CANDIDATES);
    }

    if (!result.best.valid) {
        result.statusText =
            QStringLiteral("Digital Video Hunter: no wide digital carrier, floor %1 dB, threshold +%2 dB")
    ...
    result.statusText =
        QStringLiteral("Digital Video Hunter: %1 candidate%2, best %3 at %4 MHz, width %5 MHz, flat %6 dB, occ %7%, avg +%8 dB")
            ...
            .arg(result.best.flatnessDb, 0, 'f', 1)
            .arg(result.best.occupancy * 100.0f, 0, 'f', 0)
