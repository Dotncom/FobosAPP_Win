...
struct fobos_dev_t
{
    ...
    int transfer_errors;
    volatile int transfer_pending_count;
    int dev_lost;
    int use_zerocopy;
    //=== common ===============================================================
    uint16_t user_gpo;
...
static void LIBUSB_CALL _libusb_callback(struct libusb_transfer *transfer)
{
    ...
    if (dev->transfer_pending_count > 0)
    {
        dev->transfer_pending_count--;
    }

    int canceling = dev->rx_async_cancel || FOBOS_CANCELING == dev->rx_async_status;
    if (LIBUSB_TRANSFER_COMPLETED == transfer->status)
    {
        if (!canceling && transfer->actual_length == (int)dev->transfer_buf_size)
        {
            //printf_internal(".");
            dev->rx_buff_counter++;
        ...
        if (!canceling && FOBOS_RUNNING == dev->rx_async_status)
        {
            ...
            {
                printf_internal("libusb_submit_transfer from callback returned: %d %s\n", submit_result, libusb_error_name(submit_result));
                dev->dev_lost = 1;
                fobos_rx_cancel_async(dev);
            }
        }
...
int fobos_rx_read_async(struct fobos_dev_t * dev, fobos_rx_cb_t cb, void *ctx, uint32_t buf_count, uint32_t buf_length)
{
    ...
    while (FOBOS_IDDLE != dev->rx_async_status)
    {
        if (FOBOS_CANCELING == dev->rx_async_status || dev->rx_async_cancel)
        {
            ...
            if (dev->transfer)
            {
                for (size_t i = 0; i < dev->transfer_buf_count; ++i)
                {
                    ...
                    int cancel_result = libusb_cancel_transfer(dev->transfer[i]);
                    if (cancel_result < 0 && cancel_result != LIBUSB_ERROR_NOT_FOUND)
                    {
                        printf_internal("libusb_cancel_transfer[%d] returned: %d %s\n", i, cancel_result, libusb_error_name(cancel_result));
                    }
                }

                int cancel_waits = 0;
                while (dev->transfer_pending_count > 0 && cancel_waits < 200)
                {
                ...
                if (dev->transfer_pending_count > 0)
                {
                    printf_internal("Timed out waiting for cancelled transfers: %d\n", dev->transfer_pending_count);
                }
            }
            break;
        }
...
int fobos_rx_cancel_async(struct fobos_dev_t * dev)
{
    ...
    if (FOBOS_RUNNING == dev->rx_async_status ||
        FOBOS_STARTING == dev->rx_async_status ||
        FOBOS_CANCELING == dev->rx_async_status)
    {
        dev->rx_async_status = FOBOS_CANCELING;
