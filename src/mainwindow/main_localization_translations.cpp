...
void YourClassName::loadUiTranslations() {
    ...
    uiTranslations[QStringLiteral("en")] = baseLanguage({
        ...
        {"spectrum_frames_idle", "Spectrum frames: idle"},
        {"audio_wav", "Audio WAV"},
        ...
        {"spectrum_frames_buffer_off", "Spectrum frames: buffer off"},
        {"spectrum_recording_failed", "Spectrum recording failed"},
        {"spectrum_recording", "Spectrum recording"},
        {"spectrum_saved", "Spectrum saved"},
        {"spectrum_recording_stopped_no_frames", "Spectrum recording stopped: no frames"},
        ...
        {"playback", "Playback"},
        {"record", "Record"},
        {"spectrum_rec", "Spectrum Rec"},
        {"spectrum_replay", "Replay"},
        {"spectrum_buffer", "Buffer"},
        {"spectrum_event_spectrum_only", "Spectrum only"},
        {"spectrum_event_channel_iq", "Spectrum + Channel IQ"},
        {"spectrum_event_full_iq", "Spectrum + Full IQ short"},
        {"prebuffer_short", "Pre:"},
        {"bins_short", "Bins:"},
        {"spectrum_rec_tooltip", "Record compact FFT spectrum frames for event replay and measurement."},
        {"spectrum_replay_tooltip", "Open a spectrum-frame recording and inspect the waterfall timeline."},
        {"spectrum_event_mode_tooltip", "Choose what is saved when the spectrum event trigger is pressed."},
        {"spectrum_buffer_tooltip", "Keep a rolling pre-trigger spectrum buffer in RAM."},
        {"spectrum_bins_tooltip", "Maximum bins stored per spectrum frame. Higher values preserve detail but increase file size."},
        {"spectrum_prebuffer_tooltip", "Seconds of spectrum frames kept before the hotkey trigger."},
        {"hold_f9", "Hold F9"},
        {"stop_rec", "Stop Rec"},
        ...
        {"internal", "Internal"},
        {"external", "External"},
        {"hf_cancel_lab", "HF interference lab"},
        {"hf_interference_lab_section", "HF interference lab"},
        {"volume", "Volume"},
        {"lna_gain", "LNA Gain"},
        {"vga_gain", "VGA Gain"},
        {"rtl_agc", "RTL AGC"},
        ...
        {"off", "Off"},
        {"hf_cancel", "HF cancel"},
        {"hf_baseline", "Baseline"},
        {"hf_raw_overlay", "Raw overlay"},
        {"hf_raw_overlay_tooltip", "Draw the original raw spectrum over the cleaned HF baseline graph."},
        {"hf_audio_blanker", "Audio blanker"},
        {"hf_audio_blanker_tooltip", "Blank short impulse spikes in the selected HF audio channel."},
        {"hf_audio_blanker_threshold", "Blanker threshold"},
        {"hf_audio_blanker_threshold_tooltip", "Higher values blank only stronger impulse spikes; lower values are more aggressive."},
        {"hf_baseline_depth", "Baseline depth"},
        {"hf_baseline_smooth", "Smooth"},
        {"hf_baseline_learn", "Learn"},
        {"hf_interference_defaults", "Default"},
        {"hf_interference_default_tooltip", "Reset HF interference lab controls and clear the learned baseline."},
        {"hf_baseline_empty", "Baseline: empty"},
        {"hf_baseline_ready", "Baseline ready"},
        {"hf_baseline_tooltip", "Subtract a learned HF noise-floor curve from the visual spectrum and waterfall only."},
        {"hf_baseline_learn_tooltip", "Capture the current visible spectrum as the HF interference baseline."},
        {"hf_baseline_clear_tooltip", "Clear the learned HF interference baseline."},
        {"bins", "bins"},
        {"enabled", "enabled"},
        {"ref_gain", "Ref gain"},
        {"ref_delay", "Ref delay"},
    ...
    uiTranslations[QStringLiteral("uk")] = baseLanguage({
        ...
        {"network", "Мережа"},
        {"recording_idle", "Запис: очікування"},
        {"playback_idle", "Відтворення: очікування"},
        {"spectrum_frames_idle", "Кадри спектру: очікування"},
        {"audio_wav", "Audio WAV"},
        {"channel_iq_wav", "Channel IQ WAV"},
        {"no_wav_recordings_found", "WAV-записів не знайдено"},
        {"recording_blocked_channel_iq_full_iq", "Запис заблоковано: Channel IQ не може працювати під час Full IQ-потоку"},
        {"recording_stopped_channel_iq_full_iq", "Запис зупинено: Channel IQ не може працювати під час Full IQ-потоку"},
        {"recording_failed", "Помилка запису"},
        {"recording_audio", "Запис аудіо"},
        {"recording_channel_iq_waiting", "Запис Channel IQ: очікування IQ-кадрів"},
        {"recording_channel_iq", "Запис Channel IQ"},
        {"iq_recording_failed", "Помилка IQ-запису"},
        {"recording_saved", "Запис збережено"},
        {"recording_stopped_no_data", "Запис зупинено: немає даних"},
        {"spectrum_frames_buffer_off", "Кадри спектру: буфер вимкнено"},
        {"spectrum_recording_failed", "Помилка запису спектру"},
        {"spectrum_recording", "Запис спектру"},
        {"spectrum_saved", "Спектр збережено"},
        {"spectrum_recording_stopped_no_frames", "Запис спектру зупинено: немає кадрів"},
        {"playback_blocked_stop_receiver", "Відтворення заблоковано: спочатку зупини приймач"},
        {"playback_blocked_stop_recording", "Відтворення заблоковано: спочатку зупини запис"},
        {"playback_no_file_selected", "Відтворення: файл не вибрано"},
        {"playback_failed", "Помилка відтворення"},
        {"playback_audio_wav_mono", "Помилка відтворення: аудіо WAV має бути mono 48 kHz"},
        {"playback_stopped", "Відтворення: зупинено"},
        {"playback", "Відтворення"},
        {"record", "Запис"},
        {"spectrum_rec", "Запис спектру"},
        {"spectrum_replay", "Перегляд"},
        {"spectrum_buffer", "Буфер"},
        {"spectrum_event_spectrum_only", "Тільки спектр"},
        {"spectrum_event_channel_iq", "Спектр + Channel IQ"},
        {"spectrum_event_full_iq", "Спектр + Full IQ коротко"},
        {"prebuffer_short", "До:"},
        {"bins_short", "Біни:"},
        {"spectrum_rec_tooltip", "Записати компактні FFT-кадри спектру для перегляду події та вимірювань."},
        {"spectrum_replay_tooltip", "Відкрити запис кадрів спектру і переглянути часову лінію водоспаду."},
        {"spectrum_event_mode_tooltip", "Вибір, що зберігати при натисканні тригера spectrum event."},
        {"spectrum_buffer_tooltip", "Тримати кільцевий буфер кадрів спектру перед тригером у RAM."},
        {"spectrum_bins_tooltip", "Максимум бінів у кадрі спектру. Більше значення дає деталі, але збільшує файл."},
        {"spectrum_prebuffer_tooltip", "Скільки секунд кадрів спектру тримати перед гарячою клавішею."},
        {"hold_f9", "F9 утрим."},
        {"stop_rec", "Стоп запис"},
        ...
        {"internal", "Внутр."},
        {"external", "Зовн."},
        {"hf_cancel_lab", "КХ лабораторія завад"},
        {"hf_interference_lab_section", "КХ лабораторія завад"},
        {"volume", "Гучність"},
        {"lna_gain", "LNA"},
        {"vga_gain", "VGA"},
        {"rtl_agc", "RTL AGC"},
        ...
        {"auto", "Авто"},
        {"off", "Викл."},
        {"hf_cancel", "КХ компенсація"},
        {"hf_baseline", "Підкладка"},
        {"hf_raw_overlay", "Сирий поверх"},
        {"hf_raw_overlay_tooltip", "Показати початковий сирий спектр поверх очищеного графіка КХ-підкладки."},
        {"hf_audio_blanker", "Аудіо blanker"},
        {"hf_audio_blanker_tooltip", "Вирізати короткі імпульсні сплески у вибраному КХ-аудіоканалі."},
        {"hf_audio_blanker_threshold", "Поріг blanker"},
        {"hf_audio_blanker_threshold_tooltip", "Більші значення вирізають тільки сильніші імпульси; менші працюють агресивніше."},
        {"hf_baseline_depth", "Глибина підкладки"},
        {"hf_baseline_smooth", "Згладжування"},
        {"hf_baseline_learn", "Навчити"},
        {"hf_interference_defaults", "За замовч."},
        {"hf_interference_default_tooltip", "Повернути стандартні налаштування лабораторії КХ-завад і очистити навчену підкладку."},
        {"hf_baseline_empty", "Підкладка: порожньо"},
        {"hf_baseline_ready", "Підкладка готова"},
        {"hf_baseline_tooltip", "Віднімає навчену криву шумової підкладки тільки з візуального спектра та водоспаду."},
        {"hf_baseline_learn_tooltip", "Зняти поточний видимий спектр як підкладку КХ-завад."},
        {"hf_baseline_clear_tooltip", "Очистити навчену підкладку КХ-завад."},
        {"bins", "бінів"},
        {"enabled", "увімкнено"},
        {"ref_gain", "Опора"},
        {"ref_delay", "Затримка"},
